...
enum Account: AccountCompatible {
  ...
  mutating func removeFunds(amount: Int) -> Bool {
    guard (try? self.addFunds(amount * -1)) != nil else { return false }
    return true
  }
}
...
print ("remove 1: ", account.removeFunds(15))
print ("remove 2: ", account.removeFunds(55))
// prints:
// : add:  Optional(())
// : remove 1:  true
// : remove 2:  false
...
If you look closely, you'll see that within the =JSONMappingError=, we're wrapping the *Argo* =DecodeError= into our =APIError= type as we're still using
Argo for the actual JSON decoding.
