...
You could easily fulfill this protocol with a =struct=, but in the context of your application, a =enum= is the more sensible approach. However, you can't add properties like =var remainingFunds: Int= to an =enum=, so how would you model that? The answer is actually easy, you can use associated values for this:
