...
We're also nesting an =ErrorType= compatible =enum= in the *Account* enum so that we can use Swift 2.0's new error handling. This is explained in more detail in the [[#errortype][*Practical Use Cases*]] section.
...
#+BEGIN_SRC swift :noweb strip-export :prologue "import CoreGraphics"
<<entities>>
extension Entities {
...
#+BEGIN_SRC swift :prologue "import Foundation; import UIKit" :noweb-ref cgsizenum
...
As an example, have a look at the popular [[https://github.com/thoughtbot/Argo][JSON Decoding library Argo]]. When their JSON Decoding fails, it can fail due to two primary reasons.
...
you'd instead use a =Set= of enums with associated values. That way, the type checker can confirm that your configuration values are of the correct type.
