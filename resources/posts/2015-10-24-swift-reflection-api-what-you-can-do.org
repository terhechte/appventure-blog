...
Before we do that, let's define a simple data structure that we can use as our.
...
However, we don't want to write custom serialization to Core Data code whenever we introduce a new =struct=. Rather, we'd like to develop this in a way so that we can utilize it for all future =structs= we develop.
